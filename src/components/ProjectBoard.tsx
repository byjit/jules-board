...
export function ProjectBoard() {
		...
		return (
			<div className="flex flex-col items-center justify-center h-[60vh] text-center px-4">
				...
				<p className="text-muted-foreground">
					Create a new project or select an existing one from the dropdown to
					see the board.
				</p>
			</div>
		);
	...
	const createJulesSession = async (story: UserStory, projectId: string) => {
		...
		if (
			!(project && effectiveJulesApiKey && project.gitRepo && project.gitBranch)
		) {
			if (!effectiveJulesApiKey) {
				...
				toast.info(
					"Set Git Repository and Branch in settings to enable automation.",
				);
			}
			return;
		}
	...
	const handleMoveStory = async (storyId: string, newStatus: ColumnType) => {
		...
		if (newStatus !== "todo" && story.dependsOn && story.dependsOn.length > 0) {
			const incompleteDependencies = story.dependsOn.filter((depId) => {
				return selectedProject.userStories.find(
					(s) =>
						s.id === depId && (s.status === "todo" || s.status === "doing"),
				);
			});
